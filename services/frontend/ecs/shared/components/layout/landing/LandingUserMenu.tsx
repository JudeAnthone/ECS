...
export function LandingUserMenu({ user }: { user: { name: string; email: string; avatar: string; role?: string } }) {
  ...
  return (
    <div className="w-full flex items-center gap-4 bg-[#BA0021] px-4 py-2">
      {/* Notification Bell */}
      <button
      ...
      <div className="relative">
        <button
          ref={btnRef}
          className="flex items-center justify-center w-12 h-12 rounded-full bg-white focus:outline-none"
          onClick={() => setOpen((v) => !v)}
        ...
        >
          {user.avatar ? (
            ...
            <div className="h-10 w-10 rounded-full bg-white text-[#BA0021] font-bold flex items-center justify-center text-lg">
        ...
        {open && (
          <div
            ref={menuRef}
            className="absolute z-50 mt-2 right-0 min-w-[260px] max-w-[90vw] rounded-xl shadow-2xl border border-slate-200 bg-white animate-fade-in"
            ...
            <div className="flex items-center gap-3 px-4 py-3 text-left text-sm bg-[#BA0021]/5 rounded-t-xl relative">
              {user.avatar ? (
                <img
                  src={user.avatar}
                  alt="Profile"
                  className="h-9 w-9 rounded-full object-cover border border-[#BA0021]"
                />
              ) : (
              ...
              <div className="grid flex-1 text-left text-sm leading-tight">
                <span className="truncate font-semibold text-[#BA0021]">{user.name}</span>
                <span className="truncate text-xs text-slate-500">{user.email}</span>
              </div>
              <button onClick={() => setOpen(false)} className="absolute top-2 right-2 p-1 rounded hover:bg-slate-200" aria-label="Close profile dropdown">
      ...
      {notifOpen && (
        <div className="absolute z-50 mt-2 left-0 min-w-[300px] max-w-[90vw] rounded-xl shadow-2xl border border-slate-200 bg-white animate-fade-in" style={{ top: '60px' }}>
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 rounded-t-xl">
            <span className="font-bold text-[#BA0021] text-lg">Notifications</span>
