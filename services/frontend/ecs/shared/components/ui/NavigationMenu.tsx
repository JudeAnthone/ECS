...
const navigationMenuTriggerStyle = cva(
  "group inline-flex h-9 w-max items-center justify-center rounded-md bg-background px-4 py-2 text-sm font-medium text-popover-foreground hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground disabled:pointer-events-none disabled:opacity-50 data-[state=open]:hover:bg-accent data-[state=open]:text-accent-foreground data-[state=open]:focus:bg-accent data-[state=open]:bg-accent/50 focus-visible:ring-ring/50 outline-none transition-[color,box-shadow] focus-visible:ring-[3px] focus-visible:outline-1"
)
...
function NavigationMenuTrigger({
  ...
  const triggerClass = cn(navigationMenuTriggerStyle(), "group", className)

  // If `asChild` is used we must pass a single child element to the Radix
  // trigger. Clone the child and append the chevron icon so the primitive
  // still receives a single element but the UI shows the icon.
  if (props.asChild && React.isValidElement(children)) {
    const child = React.cloneElement(children as React.ReactElement, {
      className: cn(triggerClass, (children as any).props?.className),
